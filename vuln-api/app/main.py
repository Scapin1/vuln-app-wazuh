...
def list_connections(
    ...
    return [
        {
            "id": c.id, "name": c.name, "indexer_url": c.indexer_url,
            "wazuh_user": c.wazuh_user, "version": c.version, "is_active": c.is_active
        }
        for c in conns
    ]
...
def test_wazuh_connection(
    ...
    ok = test_connection(conn.indexer_url, conn.wazuh_user, conn.wazuh_password)
...
def sync_connection(
    ...
    raw_vulns = fetch_all_vulns(conn.indexer_url, conn.wazuh_user, decrypt(conn.wazuh_password), conn.version)
    count = 0
...
def sync_all_connections(
    ...
    for conn in conns:
        try:
            raw_vulns = fetch_all_vulns(conn.indexer_url, conn.wazuh_user, conn.wazuh_password, conn.version)
            # ... mismo loop de sync ...
            results.append({"connection": conn.name, "synced": len(raw_vulns), "ok": True})
